...
## Getting Started
   ...
   3. Rename the a2f-skeleton-theme folder (something that fits the project)
...
Make sure to enqueue (or remove unneeded) any extra CSS or JS files, located in functions.php, in methods:
    ...
    A2F_Theme::enqueue_scripts()
    A2F_Theme::enqueue_styles()
...
This folder stores all theme-related widgets. Each widget can be registered in A2F_Theme::widget_init() method (functions.php). For building widgets, please see the [Widgets API article](http://codex.wordpress.org/Widgets_API)
